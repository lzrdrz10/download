    ...
    function generarCodigo() {
      ...
      if (rawVideoInput) {
        if (rawVideoInput.includes("lzrdrz10.github.io/premiumplayer/player.html")) {
          ...
          // === SOLO NOMBRE DEL CAPÍTULO (como pediste) ===
          const episodeTitleOnly = seleccionado.episodeTitle || `Episodio ${seleccionado.episodeNumber}`;
          
          const posterHorizontal = seleccionado.episodeStill || seleccionado.backdrop || seleccionado.poster || "https://raw.githubusercontent.com/lzrdrz10/premiumplayer/main/peli.jpeg";
          
          const encodedVideo = encodeURIComponent(rawVideoInput);
          const encodedPoster = encodeURIComponent(posterHorizontal);
          const encodedTitle = encodeURIComponent(episodeTitleOnly);

          videoUrl = `${PREMIUM_PLAYER_BASE}?video=${encodedVideo}&poster=${encodedPoster}&title=${encodedTitle}`;
          
          console.log("%c✅ Premium Player auto-generado (solo nombre del capítulo)", "color:#00ff7f;font-weight:bold", videoUrl);
        }
      }
...
-->`.trim();
    ...
    console.log("%c✅ PANEL LZPLAY EPISODIOS listo - Premium Player con SOLO el nombre del capítulo", "color:#00ff7f; font-size:18px");
